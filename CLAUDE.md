...
- `.scroll-banner` (= `.hero`, ten sam element) ma wysokość `400vh` (od 2026-09-02; było `300vh`). Scroll w obrębie tego tracku dzieli się na **trzy fazy** sterowane stałymi `SCRUB_END = 0.65` i `REVEAL_END = 0.8` w `banner.js`:
  1. **Scrub (0 → 65% scrolla)** — wideo się odtwarza, `videoProgress = min(1, progress/SCRUB_END)`, klatka = `round(videoProgress * (FRAME_COUNT-1))`. `.scroll-banner__scrim` ma `opacity:0` przez całą tę fazę — wideo jest wyraźne, bez przydymienia (świadomie, użytkownik nie chciał przyciemnienia podczas scrubu).
  2. **Reveal (65% → 80% scrolla)** — klatka zostaje zamrożona na ostatniej (rozłożony aparat), `revealProgress = clamp((progress-SCRUB_END)/(REVEAL_END-SCRUB_END), 0, 1)` steruje jednocześnie: `.hero__content` (`opacity` 0→1 + `translateY(24px)→0`, czyli tekst „wjeżdża” na środek) i `.scroll-banner__scrim` (`opacity` 0→1, gradient przez `.scroll-banner__scrim` — osobny `<div>`, nie `::after`, żeby JS mógł mu bezpośrednio ustawiać `style.opacity`).
  3. **Hold (80% → 100% scrolla)** — nic się już nie zmienia (`revealProgress` zostaje przy `1`, bo formuła z kroku 2 sama się clampuje powyżej `REVEAL_END`), to czysto dodatkowy dystans scrolla (~80vh) dający czas na przeczytanie w pełni ujawnionego tekstu, zanim sekcja się odepnie i strona przejdzie do „Cześć, na imię mam Robert”. Dodane na prośbę użytkownika — bez tej fazy tekst pojawiał się dokładnie w momencie, gdy sekcja i tak przestawała być przypięta, więc nie dało się go przeczytać.
  - `.hero__scroll` („Przewiń”, `#scrollBannerCue`) ma **osobną, szybką** logikę fade-out w pierwszych 5% scrolla (`1 - min(1, progress/0.05)`) — znika zaraz po starcie scrolla, niezależnie od fazy reveal/hold (jego rola to zachęta do scrollowania, nie ma sensu żeby wracał na końcu).
...
- `prefers-reduced-motion: reduce` — sekcja kurczy się do `100dvh` (CSS) i JS od razu ustawia stan spoczynkowy: **pierwsza klatka** (złożony aparat — było `FRAME_COUNT - 1`/ostatnia klatka do 2026-09-04, zmienione na prośbę użytkownika bo ostatnia klatka jako pojedyncze statyczne zdjęcie wyglądała na niedokończoną/mylącą) + `revealProgress` symulowane na `1` (tekst i scrim w pełni widoczne, żeby hero zostało czytelne) + cue ukryty. Nie testuj tej ścieżki próbą wyłączenia animacji w DevTools na tej samej karcie, którą testujesz normalnie — trzeba osobnej sesji z ustawionym `prefers-reduced-motion` (ten `mcp__Claude_Browser__*` nie ma takiej emulacji, tylko `colorScheme`).
...
- **Komunikat dla użytkowników z ograniczonym ruchem** (od 2026-09-04): `.hero__motion-notice` — akapit wewnątrz `.hero__content` tłumaczący, że animacja jest wyłączona przez ustawienie systemowe/przeglądarki, z podpowiedzią jak to zmienić. Widoczność **czysto przez CSS** (`display:none` domyślnie, `display:block` tylko pod `@media (prefers-reduced-motion: reduce)`) — nie przez JS — więc działa nawet gdyby `banner.js` się nie załadował. Nie przenoś tej logiki widoczności do JS „dla spójności” z resztą banera — to świadomy wybór odporności (progressive enhancement), zostaw jak jest.
